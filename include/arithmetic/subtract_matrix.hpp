 ...
 * wise subtraction of the `matrix` of both the `MATRIX` objects and
...
MATRIX *SubtractMatrix(MATRIX *A, MATRIX *B)
{
    ...
    // create a matrix to hold the difference of the matrix A and matrix B
    MATRIX *result = new MATRIX(A->GetRows(), A->GetCols());

    // subtract the corresponding elements of the matrix
